...
export default function Categories() {
  ...
  // ✅ FIX: localStorage hata ke /api/admin/me use karo
  const checkAuth = async () => {
    try {
      ...
      // Auth OK — ab categories load karo
      loadCategories()
    } catch {
      router.push('/admin/login')
    }
  ...
  const addCategory = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setNewCategory({ name: '', slug: '', icon: '', description: '', display_order: 0 })
        loadCategories()
      }
    } catch (error) {
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      {pagedCategories.length > 0 ? (
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="text-4xl text-teal-500 flex items-center justify-center w-12 h-12 rounded-xl bg-teal-50 dark:bg-teal-500/10">
        ...
        <div className={`text-center py-12 rounded-xl border ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'}`}>
          ...
          <p className={`text-lg font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>No categories found</p>
          <p className={isDarkMode ? 'text-slate-400' : 'text-gray-600'}>Click &quot;Add Category&quot; to create your first category</p>
        </div>
      )}
          ...
          <div className={`relative rounded-xl shadow-xl w-full max-w-lg ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}>
            ...
            <form onSubmit={addCategory}>
              <div className="p-6 space-y-4">
                {[
                  ...
                  <div key={key}>
                    ...
                    <input type={type} required={required} value={newCategory[key]} placeholder={placeholder}
                      onChange={(e) => setNewCategory({ ...newCategory, [key]: e.target.value, ...(extra ? extra(e.target.value) : {}) })}
                ...
                <div>
                  <label className={`block text-sm font-medium mb-1 ${isDarkMode ? 'text-slate-400' : 'text-gray-700'}`}>Description</label>
                  <textarea rows="3" value={newCategory.description} placeholder="Describe this category..."
                    onChange={(e) => setNewCategory({ ...newCategory, description: e.target.value })}
                    className={`w-full px-4 py-2 rounded-lg border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-gray-300 text-gray-900'} focus:outline-none focus:ring-2 focus:ring-teal-500`}
                  />
                </div>
                <div>
                  <label className={`block text-sm font-medium mb-1 ${isDarkMode ? 'text-slate-400' : 'text-gray-700'}`}>Display Order</label>
                  <input type="number" value={newCategory.display_order} placeholder="0"
              ...
              <div className="p-6 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
                ...
                <button type="submit" className="px-4 py-2 rounded-lg bg-gradient-to-r from-teal-500 to-cyan-500 text-white font-medium hover:opacity-90 transition-opacity">
                  Add Category
                </button>
              </div>
            </form>
          ...
          <div className={`relative rounded-xl shadow-xl w-full max-w-lg ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}>
            ...
            <form onSubmit={updateCategory}>
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className={`block text-sm font-medium mb-1 ${isDarkMode ? 'text-slate-400' : 'text-gray-700'}`}>Display Order</label>
                  <input type="number" value={selectedCategory.display_order}
