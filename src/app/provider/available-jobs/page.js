  ...
  const acceptJob = async (jobId) => {
    try {
      ...
      if (data.success) {
        ...
        // ✅ Redirect to /provider/jobs after successful acceptance
        setTimeout(() => {
          ...
          <div className="flex gap-1.5 bg-gray-100 p-1 rounded-xl flex-wrap">
            {[
              ...
              <button key={val} onClick={() => setFilter(val)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${filter === val ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                  }`}>
                {label}
              </button>
            ))}
            ...
            return (
              <div key={job.id} className={`bg-white rounded-2xl border shadow-sm hover:shadow-md transition-all overflow-hidden ${job.is_admin_assigned ? 'border-blue-200 hover:border-blue-300' :
                ...
                <div className="flex items-start justify-between p-5 pb-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="relative flex-shrink-0 group">
                      {job.photos?.length > 0 ? (
                        <div className="w-14 h-14 rounded-xl overflow-hidden border border-gray-100 shadow-sm relative">
                ...
                <div className="p-5 pt-0 flex gap-2">
                  ...
                  <button
                    onClick={() =>
                      handleAcceptClick(job.id, job.display_amount, hasOvertime)
                    }
